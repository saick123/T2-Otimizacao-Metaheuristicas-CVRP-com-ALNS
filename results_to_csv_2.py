...
if __name__ == "__main__":
    ...
    save_name = 'test_1.csv'
    header = 'instance; param1; param2; fsoloptimal; fmhmin; fmhmean; tempmin; tempmean; gapmin; gapmean'
    ...
    for line in lines:
        for i in range(1, len(line)):
            line[i] = f'{float(line[i]):.5f}'
    
    lines.insert(0, header) ## colocando header no começo
    
    for i, line in enumerate(lines):
        line = [x for idx, x in enumerate(line) if idx in colunas_filtro]
        lines[i] = line
